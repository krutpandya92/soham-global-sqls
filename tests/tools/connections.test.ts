...
const mgr = {
  list: vi.fn(() => [{ name: "a", engine: "mssql", database: "d", isActive: true, allow_writes: false }]),
  use: vi.fn(async (_n: string) => {}),
  activeName: vi.fn(() => "a"),
  activeAdapter: vi.fn(() => ({ dialect: "mssql", profile: { allow_writes: false, database: "d" } })),
};

describe("connection tools", () => {
  beforeEach(() => { mgr.use.mockClear(); });
