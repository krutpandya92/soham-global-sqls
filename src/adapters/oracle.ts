...
function maybeInitThick(mode: OracleProfile["mode"]): void {
  ...
  try {
    ...
    if (mode === "thick") throw new Error("[oracle] thick mode requested but Oracle Instant Client not found");
...
export class OracleAdapter implements SqlAdapter {
  ...
  async connect(): Promise<void> {
    ...
    this.pool = await oracledb.createPool({
      ...
      poolMin: 1, poolMax: 10, poolTimeout: 30,
    });
  }

  async close(): Promise<void> { if (this.pool) { await this.pool.close(0); this.pool = null; } }

  async ping(): Promise<boolean> {
    try {
      const c = await this.pool!.getConnection();
      try { await c.execute("SELECT 1 FROM DUAL"); return true; } finally { await c.close(); }
    } catch { return false; }
  }
  ...
  async listDatabases(): Promise<string[]> { return [this.profile.connectString]; }
  ...
  async listTables(schema?: string): Promise<TableInfo[]> {
    ...
    if (schema) { params.push(schema.toUpperCase()); q += " WHERE OWNER = :1"; }
    q += " ORDER BY OWNER, TABLE_NAME";
    const r = await this.runQuery(q, params);
    return r.rows.map((x) => ({ name: x.TABLE_NAME as string, schema: x.OWNER as string, type: "table" }));
  }
  ...
  async listColumns(table: string, schema?: string): Promise<ColumnInfo[]> {
    ...
    if (schema) { params.push(schema.toUpperCase()); q += " AND OWNER = :2"; }
  ...
  async listIndexes(table: string, schema?: string): Promise<IndexInfo[]> {
    ...
    const params: unknown[] = [table.toUpperCase()];
    if (schema) { params.push(schema.toUpperCase()); q += " AND i.TABLE_OWNER = :2"; }
    ...
    for (const row of r.rows) {
      ...
      else map.set(name, { name, unique: row.UNIQUENESS === "UNIQUE", columns: [row.COLUMN_NAME as string] });
    }
    return Array.from(map.values());
  }

  async sampleTable(table: string, limit: number, schema?: string): Promise<QueryResult> {
    const qualified = schema ? `${this.quoteIdent(schema.toUpperCase())}.${this.quoteIdent(table.toUpperCase())}` : this.quoteIdent(table.toUpperCase());
    return this.runQuery(`SELECT * FROM ${qualified} FETCH FIRST ${Math.max(1, Math.floor(limit))} ROWS ONLY`, [], limit);
  }
}
